...
--increase sequence
select "{{SCHEMA_NAME}}"."s_role".NEXTVAL from dual;
select "{{SCHEMA_NAME}}"."s_role".NEXTVAL from dual;
select "{{SCHEMA_NAME}}"."s_role".NEXTVAL from dual;
select "{{SCHEMA_NAME}}"."s_role".NEXTVAL from dual;
select "{{SCHEMA_NAME}}"."s_role".NEXTVAL from dual;
...
insert into "{{SCHEMA_NAME}}"."user" ("id","username","firstName","lastName", "password", "activated", "activeFlag") 
values (1, 'admin','Admin','User', '$2a$10$a3BOTytYHBko.I4aKpAPR.4b6Sr3s93mUH.UNwMyDGhdxo42sm.GW', 1, 1);

--increase sequence
select "{{SCHEMA_NAME}}"."s_user".NEXTVAL from dual;
