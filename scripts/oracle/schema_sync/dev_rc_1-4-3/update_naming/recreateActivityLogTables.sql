...
BEGIN
  ...
  FOR logTableSchema in (
    select  
        REPLACE(
                ...
                TABLE_NAME)
                str4,
        'CREATE SEQUENCE "&SCHEMA_OWNER_NAME"."sz_' || TABLE_NAME || '" INCREMENT BY 1 MAXVALUE 9999999999999999999999999999 MINVALUE 1 NOCACHE' str3,
        'DROP SEQUENCE "&SCHEMA_OWNER_NAME"."sz_' || TABLE_NAME || '" ' str2,
        'DROP TABLE "&SCHEMA_OWNER_NAME"."z_' || TABLE_NAME || '" ' str1,
        DBMS_METADATA.GET_DDL('TABLE',TABLE_NAME,'&SCHEMA_OWNER_NAME') str,
        TABLE_NAME
    from all_tables 
  ...
  LOOP
    ...
    EXECUTE IMMEDIATE ('' || logTableSchema.str3);
    EXECUTE IMMEDIATE ('' || logTableSchema.str4);
  END LOOP;
